...
const onSaveTextToClip = () => {
  ...
  if (navigator.clipboard) {
    ...
    textarea.textContent = inputText.value;
...
const onGetTextFromClip = () => {
  if (navigator.clipboard) {
    // 获取剪切板内容, 只支持此种方式(也可将数据存在2个地方, 1个剪切板, 1个自定位置, 然后再获取)
...
<template>
  <div style="width: 100%">
    <div class="container">
      <div class="title">
        {{ t("page.htmlVision.utilitarianFunction.operatingShears") }} (ShearPlate.vue)
      </div>
      <div class="content">
        <div class="box">
          <div class="inputBox">
